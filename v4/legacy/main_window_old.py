...
# from image_manager import get_image_manager <-- Removed
...
from .stubs import (
    UnifiedSettingsWindow, TemplateEditorDialog, BatchScheduleManager,
    BatchScheduleDialog, ScheduleViewTab, get_image_manager
)
...
class StreamNotifyGUI:
    ...
    def __init__(self, root, db, plugin_manager=None, bluesky_core=None):
        ...
        self.operation_mode = os.getenv("APP_MODE", "selfpost").lower()
    ...
    def setup_ui(self):
        ...
        ttk.Button(toolbar, text="?? すべて選択", command=self.select_all).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Button(toolbar, text="??? 削除", command=self.delete_selected).pack(side=tk.LEFT, padx=2)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=2)
        ...
        # In v4, we use string modes or Config settings.
        # For client mode, we are usually "manual" or "client".
        # if self.operation_mode == OperationMode.AUTOPOST:
        if self.operation_mode == "autopost":
            self.dry_run_button.config(state=tk.DISABLED)
            self.execute_post_button.config(state=tk.DISABLED)
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=2)
        ttk.Button(toolbar, text="?? 統計", command=self.show_stats).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="?? 投稿予定確認", command=self.show_schedule_view).pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="?? アプリ設定", command=self.show_app_settings).pack(side=tk.LEFT, padx=2)
        ...
        type_combo = ttk.Combobox(
            ...
            values=["全て", "?? 動画", "?? アーカイブ", "?? 放送予約", "?? 放送中", "?? 放送終了", "?? プレミア"],
            state="readonly",
            width=20
        )
        ...
        self.context_menu.add_command(label="??? 画像を設定", command=self.context_edit_image)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="??? 削除", command=self.context_delete)
    ...
    def refresh_data(self):
        ...
        # ★ Phase 3: サーバーと同期
        try:
           if hasattr(self.db, 'sync_with_server'):
               logger.info("?? サーバー同期を開始します...")
               self.status_label.config(text="サーバーと同期中...")
               self.status_label.update()

               self.db.sync_with_server()

               self.status_label.config(text="同期完了")
        except Exception as e:
           logger.error(f"? サーバー同期失敗: {e}")
           self.status_label.config(text="同期失敗")
    ...
    def fetch_rss_manually(self):
        ...
        try:
            # v4: Use WebSub Client logic via Adapter or direct Client call?
            # For now, we just Refresh Data, assuming Server handles fetching.
            # But the button says "Fetch".
            # In Phase 2, we hook this to WebSubClient.fetch_videos() or similar.

            # Since we just want to verify UI, we'll strip the logic for now.
            messagebox.showinfo("Info", "In v4 Client, fetching is handled by the Center Server.\nRefreshing local view...")
            self.refresh_data()
            return

            # Original v3 code commented out:
    ...
    def classify_youtube_live_manually(self):
        ...
        try:
            ...
            if not youtube_api_plugin:
                ...
                logger.info("?? YouTube API プラグインは導入されていません")
                return
            ...
            for video in live_videos:
                ...
                try:
                    ...
                    if video_time >= time_threshold:
                        ...
                        logger.debug(f"?? 24時間以上前（スキップ）: {video_id} ({representative_time_str})")
                except ValueError as e:
                    logger.debug(f"?? 時刻解析失敗（スキップ）: {video_id} - {e}")
                    continue

            if not filtered_videos:
                messagebox.showinfo("YouTube Live 判定", "24時間以内の Live 関連動画がありません。")
                logger.info("?? 24時間以内の Live 関連動画なし")
                return
            ...
            for video in filtered_videos:
                ...
                if not classification_result.get("success"):
                    logger.debug(f"?? 分類失敗（スキップ）: {video_id}")
                    continue
...
DB を再読込みします。"""
    ...
    def apply_filters(self):
        ...
        for video in self.all_videos:
            ...
            checked = "??" if video.get("selected_for_post") else "?"
    ...
    def edit_image_file(self, item_id):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if processed_data:
                                ...
                                logger.warning(f"?? 画像処理に失敗しました（元ファイルを使用）: {standardized_filename}")
                        except Exception as e:
                            logger.warning(f"?? 画像処理エラー（元ファイルを使用、続行）: {e}")
        ...
        ttk.Button(url_input_frame, text="?? ダウンロード", command=download_from_url).pack(side=tk.LEFT, padx=2)
    ...
    def dry_run_post(self):
        ...
        if self.operation_mode == "collect":
            messagebox.showerror(
                "エラー",
...
ログ出力のみで実際には投稿されません。
    ...
    def execute_post(self):
        ...
        if self.operation_mode == "dry_run":
...
重複投稿にご注意ください。
    ...
    def show_template_editor(self):
        ...
        try:
            ...
            # メインボタン（配信元選択）
            button_configs = [
                ("?? YouTube", _show_youtube_selector, False),
...
本当に削除してもよろしいですか？
        ...
        # 削除実行
        logger.info(f"??? {len(selected)} 件の動画削除を開始します")
            ...
            for del_video in deleted_videos:
                if del_video.get("image_filename"):
                    try:
                        ...
                        logger.warning(f"?? 画像削除に失敗: {del_video['video_id']} - {e}")
...
削除してもよろしいですか？
        ...
        # 削除実行
        logger.info(f"??? 動画削除を実行: {item_id} ({video['title'][:40]}...)")
        ...
        if result.get("success"):
            ...
            if result.get("image_filename"):
                try:
                    ...
                    logger.warning(f"?? 画像削除に失敗: {item_id} - {e}")
    ...
    def _add_youtube_video(self, input_value: str):
        ...
        try:
            ...
            if not youtube_api_plugin:
                logger.warning("?? YouTube API プラグインが有効化されていません")
            ...
            if not video_details:
                logger.warning(f"?? YouTube API での取得に失敗しました: {video_id}")
            ...
            if success:
                ...
                if thumbnail_url and self.image_manager:
                    try:
                        ...
                        logger.warning(f"?? サムネイルダウンロード失敗（継続）: {e}")
                ...
                logger.warning(f"?? DB 保存に失敗しました: {video_id}")
    ...
    def _add_niconico_video(self, input_value: str):
        ...
        try:
            ...
            if not niconico_plugin:
                logger.warning("?? ニコニコプラグインが有効化されていません")
            ...
            if not video_details:
                logger.warning(f"?? ニコニコ API での取得に失敗しました: {video_id}")
            ...
            if success:
                ...
                logger.warning(f"?? DB 保存に失敗しました: {video_id_clean}")
                messagebox.showerror("エラー", "動画情報の保存に失敗しました")
        ...
        def on_save():
            ...
            try:
                ...
                if success:
                    ...
                    logger.warning(f"?? 動画は既に存在します: {video_id}")
...
class PostSettingsWindow:
    ...
    def _build_ui(self):
        ...
        image_frame = ttk.LabelFrame(main_frame, text="??? DB画像の設定", padding=10)
        ...
        if not image_filename:
            ...
            ttk.Label(post_method_frame, text="?? DB画像がないため、URLリンクカードのみ利用可能", foreground="orange").pack(
                anchor=tk.W, padx=20
            )
        else:
            ttk.Radiobutton(
    ...
    def _display_image_preview(self, parent_frame, image_filename):
        ...
        if not PIL_AVAILABLE:
            ttk.Label(parent_frame, text="?? PIL (Pillow) がインストールされていないため、プレビューは表示できません", foreground="orange").pack(anchor=tk.W, pady=5)
            return
        ...
        try:
            ...
            if not image_path.exists():
                ttk.Label(parent_frame, text=f"?? 画像ファイルが見つかりません: {image_filename}", foreground="orange").pack(anchor=tk.W, pady=5)
                return
            ...
            ttk.Label(parent_frame, text=f"?? 画像の読み込みに失敗: {str(e)[:50]}", foreground="orange").pack(anchor=tk.W, pady=5)
    ...
    def _execute_post(self, dry_run=False):
        ...
        try:
            ...
            # v4 Logic: Use adapter
            success = self.db.post_to_bluesky(video["video_id"], dry_run=dry_run)
            if not success:
                messagebox.showerror("エラー", "投稿に失敗しました。詳細についてはログを確認してください。")
                return
