...
class UnifiedSettingsWindow:
    ...
    def _build_tab_basic(self):
        ...
        ttk.Label(main_frame, text="poll: YouTube RSSをポーリングして取得\n"
                  "websub: センターサーバーからHTTP API 経由で取得", foreground='black', font=("", 9)).grid(row=7, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))
    ...
    def _build_subtab_posting_autopost(self, parent_notebook):
        ...
        # YOUTUBE_LIVE_AUTO_POST_MODE (moved from accounts_youtube tab)
        ttk.Label(frame, text="AUTOPOST モード時に投稿する配信状態", font=("", 10, "bold")).grid(row=0, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(frame, text="all: 予約枠・配信・アーカイブすべてを投稿 \n"
                  "schedule: 予約枠のみを投稿 \n"
                  "live: 予約枠と配信開始・配信終了のみ投稿 \n"
                  "archive: アーカイブ公開のみ投稿 \n"
                  "off: LIVE関連の自動投稿を行わない", foreground='black', font=("", 9)).grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))

        # AUTOPOST_INTERVAL_MINUTES
        ttk.Label(frame, text="投稿間隔(分)", font=("", 10, "bold")).grid(row=2, column=0, sticky=tk.W, pady=3)
        ...
        ttk.Label(frame, text="再起動時のルックバック時間(分)", font=("", 10, "bold")).grid(row=4, column=0, sticky=tk.W, pady=3)
        ...
        ttk.Label(frame, text="未投稿動画の大量検知閾値(件)", font=("", 10, "bold")).grid(row=6, column=0, sticky=tk.W, pady=3)
        ...
        ).grid(row=10, column=0, columnspan=3, sticky=tk.W, pady=3)

        # プレミア配信説明
        explanation_text = "プレミア配信も投稿対象に含める（デフォルト: 有効）"
        ttk.Label(frame, text=explanation_text, foreground='black', wraplength=400, justify=tk.LEFT, font=("", 9)).grid(
            row=11, column=0, columnspan=3, sticky=tk.W, padx=(10, 0), pady=(0, 5)
        )
    ...
    def _build_subtab_live_filter(self, parent_notebook):
        ...
        ttk.Label(frame, text="以下の項目は非対応です(将来的な対応予定もありません)", font=("", 9, "bold")).pack(anchor=tk.W, pady=5)
