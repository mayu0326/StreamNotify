...
class BatchScheduleDialog(tk.Toplevel):
    ...
    def __init__(self, parent, selected_video_ids: List[str], db, schedule_mgr):
        ...
        self.schedule_preview = {}  # {video_id: scheduled_datetime}
    ...
    def _build_ui(self):
        ...
        scrollable.bind("<Configure>", _on_scroll_configure)

        self._scroll_win_id = canvas.create_window((0, 0), window=scrollable, anchor="nw")

        def _on_canvas_configure(ev):
            if ev.widget == canvas and self._scroll_win_id:
                canvas.itemconfig(self._scroll_win_id, width=ev.width)
        ...
        canvas.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=(10, 0), pady=(5, 0))
        canvas.configure(yscrollcommand=vscroll.set)

        # メインコンテンツは scrollable に pack
        # ===== フレーム1: 動画情報表示 =====
        info_frame = ttk.LabelFrame(scrollable, text="📋 対象動画", padding=10)
        ...
        # ===== フレーム2: 開始時刻設定 =====
        start_frame = ttk.LabelFrame(scrollable, text="⏰ 開始時刻設定", padding=10)
        ...
        for minutes in [5, 10, 30, 60]:
            ttk.Button(
                quick_frame,
                text=f"+{minutes}分",
                command=lambda m=minutes: self._set_start_time_from_now(m),
                width=8,
            ).pack(side=tk.LEFT, padx=2)

        # ===== フレーム3: 投稿間隔設定 =====
        interval_frame = ttk.LabelFrame(scrollable, text="📏 投稿間隔設定", padding=10)
        ...
        # ===== フレーム4: プレビュー表示 =====
        preview_title_frame = ttk.LabelFrame(scrollable, text="👁️  投稿予定プレビュー", padding=10)
        preview_title_frame.pack(fill=tk.X, padx=10, pady=5)
    ...
    def _apply_schedule(self):
        ...
        if success_count > 0:
            messagebox.showinfo(
                ...
                + (f"\n⚠️  {failed_count} 件は既に予約済みのためスキップされました。" if failed_count > 0 else ""),
            )
