...
"""
Stream notify on Bluesky - v4 テンプレート処理ユーティリティ
...
TEMPLATE_REQUIRED_KEYS = {
    ...
    "youtube_schedule": ["title", "video_url", "channel_name", "live_status", "scheduled_at"],  # ★ スケジュール枠追加
...
TEMPLATE_ARGS = {
    ...
    "twitch_online": [
        ...
        ("ゲーム名", "game_name"),
        ("配信 URL", "stream_url"),
        ("配信開始日時", "started_at | datetimeformat"),
    ],
    ...
    "twitch_offline": [
        ...
        ("配信終了日時", "ended_at | datetimeformat"),
    ],
...
# __file__ = v4/core/templates/template_utils.py
# 3階層上 (v4/) → Asset/templates/ が実際のテンプレートディレクトリ
TEMPLATE_ROOT = Path(__file__).parent.parent.parent / "Asset" / "templates"
DEFAULT_TEMPLATE_DIR = TEMPLATE_ROOT / "default"
DEFAULT_TEMPLATE_PATH = DEFAULT_TEMPLATE_DIR / "default_template.txt"
FALLBACK_TEMPLATE_PATH = DEFAULT_TEMPLATE_DIR / "default_template.txt"
...
def get_template_path(
    ...
    attr_name = f"template_{template_type.lower()}_path"
    if hasattr(settings, attr_name):
        env_path = getattr(settings, attr_name)
        if env_path:
            return env_path
    ...
    # 例: "youtube_new_video" → "Asset/templates/youtube/yt_new_video_template.txt"
    # 例: "nico_new_video"    → "Asset/templates/niconico/nico_new_video_template.txt"
    parts = template_type.split("_")
    if len(parts) >= 2:
        service_name = parts[0]
        event_type = "_".join(parts[1:])
        ...
        template_path = TEMPLATE_ROOT / service_dir / template_filename
...
def load_template_with_fallback(
    ...
    try:
        ...
        env.filters["datetimeformat"] = _format_datetime_filter
