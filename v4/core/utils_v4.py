...
# from v4.thumbnails.niconico_ogp_backfill import backfill_niconico, fetch_thumbnail_url
...
def format_datetime_filter(iso_datetime_str, fmt="%Y-%m-%d %H:%M %Z"):
    ...
    try:
        ...
        # settings からタイムゾーンを取得（未指定ならシステムローカル）
        target_tz_name = settings.timezone
        target_tz = None
