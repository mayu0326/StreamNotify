...
class BlueskyPlugin:
    ...
    def __init__(self, username: str, password: str, dry_run: bool = False):
        ...
        if dry_run:
            logger.info(
                "🧪 BlueskyPlugin は DRY RUN モードで動作します。実際には投稿しません。"
            )
        else:
            self._login()
    ...
    def _build_facets(self, text: str) -> list | None:
        """
        ...
        """
        try:
            from atproto.models.com.atproto.richtext import Facet  # type: ignore
            from atproto.models.com.atproto.richtext.facet import Link  # type: ignore
        except ImportError as e:
        ...
        for url_info in urls:
            try:
                ...
                facet = Facet(
                    ...
                    features=[Link(uri=url_info["url"])],
                )
                facets.append(facet)
                logger.info(
                    f"  ✅ Facet 作成: bytes {url_info['byte_start']}-{url_info['byte_end']} → {url_info['url']}"
                )
            except Exception as e:
    ...
    def post_video(self, video: dict) -> bool:
        ...
        try:
            ...
            try:
                ...
                if facets:
                    logger.info(
                        f"📍 投稿: text={len(post_text)} 文字, facets={len(facets)} 個"
                    )
                    logger.info(f"   詳細: {[f.index for f in facets]}")

                    # AT Protocol の仕様に従って送信
                    _response = self.client.send_post(text=post_text, facets=facets)
                    logger.info(f"✅ Bluesky に投稿しました（リンク化）: {video_url}")
                else:
                    logger.warning("⚠️ Facet なしで投稿します")
                    _response = self.client.send_post(text=post_text)
