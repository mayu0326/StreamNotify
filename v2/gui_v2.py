...
class StreamNotifyGUI:
    ...
    def setup_ui(self):
        ...
        self.tree.configure(yscrollcommand=cast(Any, scrollbar.set))
    ...
    def edit_scheduled_time(self, item_id: str):
        """予約日時をダイアログで編集"""
        videos = self.db.get_all_videos()
        ...
        if video.get("scheduled_at"):
            try:
                selected_date = datetime.fromisoformat(
                    cast(str, video.get("scheduled_at"))
                ).date()
            except:
        ...
        def set_quick_time(minutes_offset: int):
            """クイック設定で時刻を更新"""
    ...
    def context_edit_scheduled(self):
        ...
        if hasattr(self, "current_context_item"):
            self.edit_scheduled_time(cast(str, self.current_context_item))

    def context_edit_image(self):
        """コンテキストメニューから画像を設定"""
        if hasattr(self, "current_context_item"):
            self.edit_image_file(cast(str, self.current_context_item))

    def context_deselect(self):
        """コンテキストメニューから選択解除"""
        if hasattr(self, "current_context_item"):
            item_id = cast(str, self.current_context_item)
            self.db.update_selection(
                item_id,
                selected=False,
                scheduled_at=None,
    ...
    def edit_image_file(self, item_id: str):
        ...
        def preview_image():
            ...
            if has_pillow:
                try:
                    with Image.open(image_path) as img_obj:
                        transposed_img = ImageOps.exif_transpose(img_obj)
                        transposed_img.thumbnail((480, 320), Image.Resampling.LANCZOS)

                        # Mypy対策: 別の変数に代入して ImageFile との不整合を回避
                        img_to_show = transposed_img
                        photo = ImageTk.PhotoImage(img_to_show)
                        img_label.configure(image=photo)
                        setattr(img_label, "image", photo)  # GC防止
                except Exception as e:
                    ttk.Label(
                        preview_window,
                        text=f"画像の読み込みに失敗しました: {e}",
                        foreground="red",
                    ).pack(pady=5)
            else:
                ttk.Label(
                    preview_window,
                    text="Pillow が未インストールのため画像表示できません。\n`pip install Pillow` を実行してください。",
                    foreground="red",
                    wraplength=480,
                ).pack(pady=5)
            ...
            # 情報を表示
            ttk.Label(info_frame, text=f"ファイル名:", font=("Arial", 9, "bold")).grid(
            ...
            ttk.Label(info_frame, text=f"形式:", font=("Arial", 9, "bold")).grid(
            ...
            ttk.Label(
                info_frame, text=f"ファイルサイズ:", font=("Arial", 9, "bold")
            ...
            if image_info.get("width") and image_info.get("height"):
                ttk.Label(info_frame, text=f"解像度:", font=("Arial", 9, "bold")).grid(
                ...
                ttk.Label(info_frame, text=f"モード:", font=("Arial", 9, "bold")).grid(
                ...
                ttk.Label(
                    info_frame,
                    text="※ Pillow未インストールのため解像度情報は取得できません",
                    foreground="gray",
                    wraplength=350,
                ).grid(row=3, column=0, columnspan=2, pady=10)

            ttk.Label(info_frame, text=f"パス:", font=("Arial", 9, "bold")).grid(
...
重複投稿にご注意ください。
    ...
    def show_plugins(self):
        ...
        text_widget.configure(yscrollcommand=cast(Any, scrollbar.set))
    ...
    def _display_image_preview(self, parent_frame, image_filename):
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                setattr(preview_label, "image", photo)  # ガベージコレクション対策
                preview_label.pack()
