        ...
        try:
            ...
            for video in live_videos:
                ...
                if live_status == "completed" or content_type == "archive":
                    ...
                    # ここでは、ライブ終了検知時にアーカイブとして確定した時点で、
                    # ライブ判定オフ設定であっても「通常の動画として通知」するために投稿処理を検討するが、
                    # ユーザーの「通知フローに乗せる」は「GUIの投稿リストに載る」ことを指すことが多いため、
                    # 自動投稿 (auto_post_live_end) は設定に従い、DB更新によってGUI側に反映させる。
                    if self.live_auto_post_end:
                        self.auto_post_live_end(video)
                    else:
                        logger.info(
                            "ℹ️ YOUTUBE_LIVE_AUTO_POST_END=false のため自動投稿をスキップ（アーカイブとしてDB更新のみ実施）"
                        )
