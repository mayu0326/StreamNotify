...
class AssetManager:
    ...
    def deploy_templates(self, services: Optional[list] = None) -> int:
        """
        テンプレートをコピー
        ...
        for service in services:
            ...
            if not src_service_dir.exists():
                logger.debug(
                    f"テンプレートディレクトリが見つかりません: {src_service_dir}"
                )
                continue
    ...
    def deploy_images(self, services: Optional[list] = None) -> int:
        """
        画像をコピー
    ...
    def deploy_plugin_assets(self, plugin_name: str) -> dict:
        ...
        Args:
            plugin_name: プラグイン名（例: "bluesky_plugin", "youtube_api_plugin"）

        Returns:
            {"templates": コピーしたテンプレート数, "images": コピーした画像数}
        """
        logger.debug(f"🔌 プラグイン '{plugin_name}' のアセット配置を開始します...")
