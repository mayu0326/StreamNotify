...
class BlueskyMinimalPoster:
    ...
    def _fetch_ogp_data(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not og_title:
                ...
                title = str(og_title.get("content", "No title"))

            description = str(og_desc.get("content", "")) if og_desc else ""
            image_url_val = og_image.get("content", None) if og_image else None
            image_url: Optional[str] = str(image_url_val) if image_url_val else None
            ...
            if ogp_data:
                title_val = str(ogp_data.get("title", ""))
                post_logger.info(f"✅ OGP 取得成功: title={title_val[:30]}...")
            return ogp_data
    ...
    def _build_external_embed(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 画像がある場合、アップロード
            image_url = ogp_data.get("image_url")
            if image_url:
                blob = self._upload_ogp_image_blob(image_url)
                if blob:
