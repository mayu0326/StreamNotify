...
def load_template_with_fallback(
    path: str, default_path: str | None = None, template_type: str = "unknown"
) -> Optional[Any]:
    """
...
def get_sample_context(template_type: str) -> Dict[str, Any]:
    ...
    template_vars = sample_contexts.get(
        template_type,
        {
            "title": "サンプルタイトル",
    ...
    return dict(template_vars) if isinstance(template_vars, dict) else {}
...
def preview_template(
    template_type: str, template_text: str, event_context: Dict[str, Any] | None = None
) -> Tuple[bool, str]:
    """
...
def save_template_file(
    template_type: str, template_text: str, output_path: str | None = None
) -> Tuple[bool, str]:
    """
    ...
    try:
        if not output_path:
            output_path_val = get_template_path(template_type)
        else:
            output_path_val = output_path

        if not output_path_val:
            error_msg = (
                f"❌ テンプレート保存先が決定できません（種別: {template_type}）"
            )
            logger.error(error_msg)
            return False, error_msg

        output_path_obj = Path(output_path_val)
        output_path_obj.parent.mkdir(parents=True, exist_ok=True)

        with open(output_path_obj, "w", encoding="utf-8") as f:
            f.write(template_text)

        success_msg = f"✅ テンプレートを保存しました: {output_path_obj}"
