...
def resize_image(file_path: str, config: Optional[Dict[str, Any]] = None) -> Optional[bytes]:
    """
    画像をリサイズして最適化
    ...
    try:
        ...
        # ========== JPEG 出力（初期品質） ==========
        jpeg_data: Optional[bytes] = _encode_jpeg(resized_img, config["quality_initial"])
...
def _optimize_image_quality(img: Image.Image, config: Dict[str, Any]) -> Optional[bytes]:
    """
