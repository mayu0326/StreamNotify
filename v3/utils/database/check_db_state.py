...
c.execute(
    "SELECT video_id, title, source, content_type FROM videos WHERE source = 'YouTube' LIMIT 5"
)
for row in c.fetchall():
...
if null_count > 0:
    c.execute(
        "SELECT video_id, title, content_type FROM videos WHERE source IS NULL LIMIT 3"
    )
    for row in c.fetchall():
