...
def _extract_key_block_from_example(
    example_lines: List[Tuple[str, str]], new_key: str
) -> List[str]:
    """
...
def sync_settings_env(
    settings_env: str = "settings.env", example_file: str = "settings.env.example"
) -> bool:
    """
    ...
    for new_key in sorted(missing_keys):
        ...
        # 挿入位置を特定
        insert_pos, target_section = _find_insertion_point(
            existing_lines, example_lines, new_key
        )
    ...
    try:
        ...
        logger.info(
            f"✅ settings.env を更新しました。{len(missing_keys)} 個の新規項目を追加しました。"
        )
        return True
